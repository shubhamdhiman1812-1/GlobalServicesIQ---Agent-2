...
class Config:
    ...
    # LLM model config: unsupported tokens/temperature for certain models
...
def _initialize_config():
    ...
    # Azure AI Search variables (not used in this agent, but pattern shown)
    ...
    # All config variables referenced in agent or required by platform
    CONFIG_VARIABLES = [
        # General
        "ENVIRONMENT",
        # Key Vault Auth (Service Principal, only if not using DefaultAzureCredential)
        "AZURE_TENANT_ID",
        "AZURE_CLIENT_ID",
        "AZURE_CLIENT_SECRET",
        # LLM / Model
        ...
        "ANTHROPIC_API_KEY",
        "GOOGLE_API_KEY",
        # Azure Translator (domain-specific)
